...
app.put('/feeditem/:feeditemid/content',function(req,res) {
  ...
  if (fromUser === feedItem.contents.author) {
    // Check that the body is a string, and not somothing like a Json object
...
app.delete('/feeditem/:feeditemid', function(req, res) {
  ...
  if (feedItem.contents.author === fromUser) {
    ...
    // Send a blank response to indicate succces.
    res.send();
  } else {
    //401: Unauthorized.
    res.status(401).end();
...
app.post('/search',function(req,res) {
  ...
  if (typeof(req.body) === 'string') {
    ...
    res.send(feedItemIDs.filter((feedItemID) => {
      var feedItem = readDocument('feedItems', feedItemID); return feedItem.contents.contents
                     .toLowerCase()
                     .indexOf(queryText) !== -1;
    }).map(getFeedItemSync));
  } else {
